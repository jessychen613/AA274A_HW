...
from std_msgs.msg import String, Float32MultiArray, Int64, Float32
import tf
import numpy as np
...
#        self.stop_min_dist = rospy.get_param("~stop_min_dist", 0.5)
        self.stop_min_dist = rospy.get_param("~stop_min_dist", 1.0)        
        ...
        self.goal_publisher = rospy.Publisher('/reachgoal', Float32, queue_size=10)
        self.reachgoal = 0.0
        self.goal_publisher.publish(self.reachgoal)
...
#        print("detection",name,confidence,dist,thetaleft,thetaright)
    ...
    def has_stopped(self):
        ...
        print("has_stopped", (self.mode == Mode.STOP and rospy.get_rostime() - self.stop_sign_start > rospy.Duration.from_sec(self.stop_time)))
    ...
    def object_location_callback(self,msg):
        self.objlocation = msg.data
#        print("get objlocation", self.objlocation )
...
#                    self.mode = Mode.TRACK                
                ...
                if self.near_goal() and (self.mode != Mode.STOP) and (self.mode != Mode.CROSS):
                    self.switch_mode(Mode.PARK)
                    #near the goal, plan next target
                    print("**********near the goal, post the reachgoal = 1.0")
                    reachedgoal = 1.0
                    self.goal_publisher.publish(reachedgoal)
                ...
                if self.at_goal():
                    ...
                    self.theta_g = None
                    self.switch_mode(Mode.IDLE)
                    #at the goal, plan next target
                    print("**********at the goal, post the reachgoal = 1.0")
                    reachedgoal = 1.0
                    self.goal_publisher.publish(reachedgoal)                    
