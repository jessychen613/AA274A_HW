...
#        model_path = rospy.get_param("~model_path", "tfmodels/stop_signs_gazebo.pb")
        ...
        self.detec_min_dist = rospy.get_param("~detec_min_dist", 1.3)
...
#        self.robstartthe = 0.0
        ...
        self.objid_publisher = rospy.Publisher('/obj_rescueid', Float32, queue_size=10)
        self.objidlist = 0.0
        self.objid_publisher.publish(self.objidlist)

        self.rescuecommand_publisher = rospy.Publisher('/rescuecommand', Float32, queue_size=10)
        self.rescuecommand = 0.0
            ...
            if len(self.detectedobjinfo)==0:
                ...
                for i in range(len(self.detectedobjinfo)):
                    if object_msg.id == self.detectedobjinfo[i].objcategory:
                        ...
                        objxpre = self.detectedobjinfo[i].objlocationx
                        objypre = self.detectedobjinfo[i].objlocationy
...
#                        if (self.objx > (objxmea-self.params.detec_loc_dev) and self.objx < (objxmea+self.params.detec_loc_dev)) and (self.objy > (objymea-self.params.detec_loc_dev) and self.objy < (objymea+self.params.detec_loc_dev)):
                        if (self.objx > (objxpre-self.params.detec_loc_dev) and self.objx < (objxpre+self.params.detec_loc_dev)) and (self.objy > (objypre-self.params.detec_loc_dev) and self.objy < (objypre+self.params.detec_loc_dev)):
...
#                            self.detectedobjinfo[i].objlocationy = self.objy
                            self.detectedobjinfo[i].objlocationx = objxmea
                            self.detectedobjinfo[i].objlocationy = objymea
...
#                            self.detectedobjinfo[i].objxlist.append(objxmea)
#                            self.detectedobjinfo[i].objylist.append(objymea)
#                            self.drop_marker(1, self.detectedobjinfo[i].objid, self.objx, self.objy, object_msg.name)
                            self.drop_marker(1, self.detectedobjinfo[i].objid, objxmea, objymea, object_msg.name)                            
                            break
                        else:
...
#        self.z = msg.pose.pose.position.z
    ...
    def navinfo_callback(self, msg):
        ...
        if (self.startcount==0 and msg.x!=0.0):
            ...
            marker.pose.orientation.z = 0.0
            marker.pose.orientation.w = 1.0
            marker.scale.x = 0.2  # artifact of sketchup export
            marker.scale.y = 0.2  # artifact of sketchup export
            marker.scale.z = 0.2  # artifact of sketchup export
