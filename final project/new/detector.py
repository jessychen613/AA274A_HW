...
from std_msgs.msg import Float32MultiArray, Float32, String, Int64
...
#        model_path = rospy.get_param("~model_path", "tfmodels/stop_signs_gazebo.pb")
        ...
        self.detec_min_dist = rospy.get_param("~detec_min_dist", 0.8)

        #Location calculate deviation
        self.detec_loc_dev = rospy.get_param("~detec_loc_dev", 0.6)
...
#        self.z = 0.
    ...
    def save_object_list(self,object_msg):
        ...
        if (object_msg.distance>0.0 and object_msg.distance<self.params.detec_min_dist):
            ...
            if len(self.detectedobjinfo)==0:
                ...
                newobj.robotx = self.x
                newobj.roboty = self.y
                newobj.robottheta = np.arctan2(self.y,self.x)
                ...
                for i in range(len(self.detectedobjinfo)):
                    if object_msg.id == self.detectedobjinfo[i].objcategory:
                        ...
                        if (self.objx > (objxmea-self.params.detec_loc_dev) and self.objx < (objxmea+self.params.detec_loc_dev)) and (self.objy > (objymea-self.params.detec_loc_dev) and self.objy < (objymea+self.params.detec_loc_dev)):
                            ...
                            self.detectedobjinfo[i].robotx = self.x
                            self.detectedobjinfo[i].roboty = self.y
                            self.detectedobjinfo[i].robottheta = np.arctan2(self.y,self.x)
        ...
        self.debugger = rospy.Publisher('/detector/objlocation', Float32MultiArray, queue_size=10)
        self.debugger.publish(data=self.objlocation)
...
#                self.objx=self.x + x_base_cam*np.cos(self.theta) - y_base_cam*np.sin(self.theta) 
 #               self.objy=self.y + x_base_cam*np.sin(self.theta) + y_base_cam*np.cos(self.theta) 
                self.objx=self.x + x_base_cam*np.cos(theta) - y_base_cam*np.sin(theta) 
                self.objy=self.y + x_base_cam*np.sin(theta) + y_base_cam*np.cos(theta)  
