...
#        model_path = rospy.get_param("~model_path", "tfmodels/stop_signs_gazebo.pb")
        ...
        self.detec_min_dist = rospy.get_param("~detec_min_dist", 1.5)

        #Location calculate deviation
        self.detec_loc_dev = rospy.get_param("~detec_loc_dev", 0.8)
...
#        self.debugger2 = rospy.Publisher('/detector/debugger2', String, queue_size=10)
        ...
        self.obj_stop_pub = rospy.Publisher("/obj_stop", String, queue_size=10)
...
#        if (object_msg.distance>0.0) and (object_msg.distance<self.params.detec_min_dist) and (self.mode!=0) and (self.mode!=1):
        print("**************object_msg.id, object_msg.name", object_msg.id, object_msg.name)
        if (object_msg.id==13.0):
            print("#########detected stop sign###############")
            self.avoid_object_collision()
        if (self.mode/1.0==1.0):
            print("@@@@@@@@@@@@ Align @@@@@@@@@@@@@@@")
        if ((object_msg.distance>0.0) and (object_msg.distance<self.params.detec_min_dist) and (self.mode!=1)):
...
#                self.objlocation.append(np.arctan2(self.y,self.x))
    ...
    def avoid_object_collision(self):
        """
        publishes zero velocities upon rospy shutdown
        """
        print("$$$$$$$Object collision avoid$$$$$$")
        command = 'stop'
        self.obj_stop_pub.publish(command)
