...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        ...
        self.closed_set = set()    # the set containing the states that have been visitedis_free
    ...
    def get_neighbors(self, x):
        ...
        if self.is_free(self.snap_to_grid((x[0]-self.resolution, x[1]))):
            neighbors.append(self.snap_to_grid((x[0]-self.resolution, x[1])))
        if self.is_free(self.snap_to_grid((x[0]+self.resolution, x[1]+self.resolution))):
...
#        print("neighbors",neighbors)
    ...
    def solve(self):
        ...
        #O:INIT(xinit)
        self.open_set.add(self.x_init)
        #SET COST TO ARRIVE SCORE(xinit; 0)
        self.cost_to_arrive[self.x_init]=0
        #SET EST COST THROUGH(xinit; DISTANCE(xinit; xgoal))
        self.est_cost_through[self.x_init]=self.distance(self.x_init, self.x_goal)

        #while O:SIZE > 0 do
        while len(self.open_set) > 0:
            #xcurrent   LOWEST EST COST THROUGH(O)
            xcurrent = self.find_best_est_cost_through()
            #if xcurrent = xgoal then return RECONSTRUCT PATH end if
...
#            print("open_set", self.open_set)

            #C:ADD(xcurrent)
            self.closed_set.add(xcurrent)
#            print("closed_set", self.closed_set)

            #for xneigh in NEIGHBORS(xcurrent) do
...
#                print("self.distance(xcurrent,xneigh)",self.distance(xcurrent,xneigh))
#                print("self.cost_to_arrive(xcurrent)",self.cost_to_arrive[xcurrent])
                #tentative cost to arrive = GET COST TO ARRIVE(xcurrent) + DISTANCE(xcurrent; xneigh)
                ten_cost_arr = self.cost_to_arrive[xcurrent] + self.distance(xcurrent,xneigh)
#                print("ten_cost_arr",ten_cost_arr)
                # if xneigh not in O then O:ADD(xneigh)
                if xneigh not in self.open_set:
                    self.open_set.add(xneigh)
                #else if tentative cost to arrive > GET COST TO ARRIVE(xneigh) then
                elif ten_cost_arr > self.cost_to_arrive[xneigh]:
#                    print("continue")
                    continue
                ...
                #SET COST TO ARRIVE(xneigh; tentative cost to arrive)
                self.cost_to_arrive[xneigh]=ten_cost_arr

                #SET EST COST THROUGH(xneigh; tentative cost to arrive + DISTANCE(xneigh; xgoal))
#                print("ten_cost_arr+self.distance(xneigh,self.x_goal)",ten_cost_arr+self.distance(xneigh,self.x_goal))
                self.est_cost_through[xneigh]=ten_cost_arr+self.distance(xneigh,self.x_goal)
